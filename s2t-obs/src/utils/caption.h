...
static PluginSettings default_CaptionPluginSettings() {
...
static bool isValidWordReplacementTypeString(const std::string &type) {
...
static void enforce_CaptionPluginSettings_values(PluginSettings &settings) {
...
static void set_SceneCollectionSettings_on_data(obs_data_t *save_data, const SceneCollectionSettings &scene_collection_settings) {
    ...
    std::string caption_when = mute_setting_to_string(caption_source_settings.mute_when, "");
...
static CaptionPluginSettings get_CaptionPluginSettings_from_data(obs_data_t *load_data) {
    ...
    if (!load_data) {
        spdlog::info("no data, using default CaptionPluginSettings");
        return settings;
    }
...
// #endif
    ...
    auto word_reps = std::vector<Filter>();
...
//                printf("adding fallback %s\n", word.c_str());
                word_reps.push_back(Filter("text_case_insensitive", word, ""));
            }
        }
    }
...
static void set_CaptionPluginSettings_on_data(obs_data_t *save_data, const PluginSettings &settings) {
...
static bool get_WordReplacements(obs_data_t *load_data, std::vector<Filter> &word_placements) {
...
//    printf("obs_data_array_count(words_array) %d\n", (int) obs_data_array_count(words_array));
    for (size_t index = 0; index < obs_data_array_count(words_array); index++) {
        ...
        std::string type = obs_data_get_string(a_scene_source_obj, "type");
        std::string from = obs_data_get_string(a_scene_source_obj, "from");
        std::string to = obs_data_get_string(a_scene_source_obj, "to");
...
static bool set_WordReplacements(obs_data_t *save_data, const std::vector<Filter> &word_placements) {
...
static PluginSettings load_CaptionPluginSettings(obs_data_t *load_data) {
...
static void save_CaptionPluginSettings(obs_data_t *save_data, const PluginSettings &settings) {
    spdlog::info("obs save event");
...
static bool save_plugin_data_to_config(obs_data_t *data) {
    ...
    if (!module_path) {
        spdlog::error("obs_module_config_path failed, couldn't get captions settings dir for save");
        return false;
    }
    os_mkdirs(module_path);

    BPtr<char> config_file_path = obs_module_config_path("s2t_closed_captions_settings.json");
    if (!config_file_path) {
        spdlog::error("obs_module_config_path failed, couldn't get captions settings file for save");
        return false;
    }
...
static bool save_CaptionPluginSettings_to_config(const PluginSettings &settings) {
    ...
    if (ok)
        spdlog::debug("s2t_caption_plugin saved settings to config file");
    else
        spdlog::debug("s2t_caption_plugin config file save failed");
...
static PluginSettings load_CaptionPluginSettings_from_config() {
    obs_data_t *loaded_data = load_plugin_data_from_config();
    if (loaded_data) {
        PluginSettings settings = get_CaptionPluginSettings_from_data(loaded_data);
        obs_data_release(loaded_data);
        spdlog::debug("s2t_caption_plugin loaded settings from config file");
        return settings;
    }

    spdlog::debug("s2t_caption_plugin no plugin config file found, using default settings");
...
static void update_combobox_output_target(QComboBox &comboBox, bool streaming_enabled, bool recording_enabled,int default_index) {
